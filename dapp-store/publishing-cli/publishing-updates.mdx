...
Run the same CLI command as the "Create the release NFT" step from the [Mint a release NFT](/dapp-store/publishing-cli/submit#mint-a-release-nft) section in these docs.
...
By submitting this command  you agree to, and represent that you have read, (i) the [Solana Mobile dApp Store Developer Agreement](/dapp-store/agreement), including the arbitration clause linked [here](/dapp-store/agreement#disputes), meaning that, where enforceable, any dispute related to the dApp Store will be resolved through BINDING ARBITRATION on an individual, non-class basis, and (ii) the [Solana Mobile Publisher Policy](/dapp-store/publisher-policy).
