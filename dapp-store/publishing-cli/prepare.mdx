---
title: "Prepare your dApp for publishing"
description: "To prepare for publishing, you must"
sidebarTitle: "2. Prepare for publishing"
---

<Steps>
<Step>
Collect your publishing assets into a folder
</Step>
<Step>
Build and sign your Android APK
</Step>
<Step>
Populate the `config.yaml`
</Step>
</Steps>
...
<CardGroup>
  <Card
    title="Solana Mobile Example Repo"
    horizontal
    icon="mobile"
    href="https://github.com/solana-mobile/dapp-publishing/tree/main/example"
  />
  <Card
    title="Gem Wallet Open Source Repo"
    horizontal
    icon="wallet"
    href="https://github.com/gemwalletcom/solana-mobile-publishing/blob/main/config.yaml"
  />
</CardGroup>
...
- Videos Requirements
  ...
  - All videos provided must be `.mp4` video file format.
      - If you have `.mp4` videos included, make sure you have `ffmpeg` utility library [installed](/dapp-store/publishing-cli/setup#ffmpeg).
...
If you're publishing an Expo app, view this [APK building and signing guide](/dapp-store/build-and-sign-an-apk).
...
<Step>
Remove any fields that don't apply (for e.g., `saga_features`, `google_store_package`, etc).
</Step>
<Step>
Do **not** modify or remove the `address` fields. These will be automatically populated by the CLI in later steps.
</Step>
<Step>
You may need to provide details in the `testing_instructions` field of the configuration file that you would not want published on-chain (e.g `test_username` and `test_password`). Rest assured, this data is not published as part of the on-chain metadata.
</Step>
</Steps>
<Tip>
**REFERENCE**
...
<Accordion title="Publishing Config Explanation">
