...
Run the same CLI command as the "Create the release NFT" step from the [Mint a release NFT](/dapp-publishing/submit#mint-a-release-nft) section in these docs.
...
By submitting this command  you agree to, and represent that you have read, (i) the [Solana Mobile dApp Store Developer Agreement](/dapp-publishing/agreement), including the arbitration clause linked [here](/dapp-publishing/agreement#disputes), meaning that, where enforceable, any dispute related to the dApp Store will be resolved through BINDING ARBITRATION on an individual, non-class basis, and (ii) the [Solana Mobile Publisher Policy](/dapp-publishing/publisher-policy).
